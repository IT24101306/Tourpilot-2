...
export function AgencyDomainPage() {
  const { token } = useAuth();
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const data = await api<DomainInfo>("/agencies/mine/domain", { token });
      setInfo(data);
  ...
  async function saveDomain(e: FormEvent) {
    ...
    setMsg("");
    try {
      const data = await api<DomainInfo>("/agencies/mine/domain", {
        method: "POST",
        token,
  ...
  async function verify() {
    ...
    setMsg("");
    try {
      const data = await api<DomainInfo>("/agencies/mine/domain/verify", {
        method: "POST",
        token,
      });
      setInfo(data);
      setMsg(
        data.status === "ACTIVE"
          ? "Verified. Your storefront is now live on your domain (HTTPS is issued automatically on the first visit)."
          : "Not verified yet."
      );
    } catch (e) {
  ...
  return (
    <div className="module-shell module-operations">
      <ModuleHeader
        module="operations"
        title="Custom domain"
        subtitle="Serve your storefront on your own domain, like myagency.com. HTTPS is set up automatically."
      />
        ...
        <div className="domain-panel">
          ...
          <div className="domain-dns-guide">
            ...
            <p className="muted">
              ...
              following DNS record. This tells browsers where to find your storefront.
            </p>
            ...
            <p className="muted" style={{ marginTop: 8, fontSize: "0.85rem" }}>
              If you are using a subdomain like <code>tours.yourdomain.com</code>, set the Name/Host
              to <code>tours</code> instead of <code>@</code>. DNS changes can take a few minutes to
